// background.js — PasteToWrite v3.0

let typingState = null;
// typingState = { tabId, stopRequested, pauseRequested, resumeResolve, wpm, errorRate, burstiness }

function rand(a, b) { return a + Math.random() * (b - a); }
function sleep(ms)  { return new Promise(r => setTimeout(r, ms)); }

// Breaks sleep into 30ms chunks so pause & setting changes apply immediately
...
// ── Main loop — reads settings from typingState on EVERY character ────────────
async function startTyping(tabId, text, wpm, errorRate, burstiness) {
  typingState = { tabId, stopRequested:false, pauseRequested:false, resumeResolve:null,
                  wpm, errorRate, burstiness };
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg.action === "updateSettings") {
    // Live-update settings while typing or paused
    if (typingState) {
  ...
  if (msg.action === "resumeTyping") {
    if (typingState && typingState.pauseRequested) {
      ...
      if (typingState.resumeResolve) { typingState.resumeResolve(); typingState.resumeResolve = null; }
